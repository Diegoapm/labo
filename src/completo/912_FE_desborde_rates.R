...
AgregarVariables  <- function( dataset )
{
  ...
  dataset[ , mv_msaldototal_by_antig     := mv_msaldototal  / cliente_antiguedad ]  
  dataset[ , mv_msaldodolares_by_antig   := mv_msaldodolares  * cliente_antiguedad ]
  dataset[ , avg_mautos_ctarjdeb_trx     := mautoservicio  / ctarjeta_debito_trx ]
  dataset[ , avg_mautos_ctar_debito_trx  := mcuentas_saldo  / avg_mautoserv_ctarjdeb_trx ]
  dataset[ , avg_mautos_ctar_debito_trx  := mautoservicio  / ctarjeta_debito_trx ]
  dataset[ , mpayroll_by_cpayroll_trx    := mpayroll  / cpayroll_trx ]
  dataset[ , monto_mvc_autoddeb          := rowSums( cbind( mcuenta_debitos_automaticos,  mtarjeta_visa_debitos_automaticos, mttarjeta_master_debitos_automaticos) , na.rm=TRUE ) ]
  dataset[ , cant_mvc_autoddeb           := rowSums( cbind( ccuenta_debitos_automaticos,  ctarjeta_visa_debitos_automaticos, ctarjeta_master_debitos_automaticos) , na.rm=TRUE ) ]
  dataset[ , mont_by_cant_autodeb        := monto_mvc_autoddeb  / cant_mvc_autoddeb ]
  dataset[ , ctransf_total               := rowSums( cbind( ctransferencias_recibidas,  ctransferencias_emitidas) , na.rm=TRUE ) ]
  dataset[ , mtransf_total               := rowSums( cbind( mtransferencias_recibidas,  mtransferencias_emitidas) , na.rm=TRUE ) ]
  dataset[ , mont_by_cant_transf         := monto_mvc_autoddeb  / cant_mvc_autoddeb ]
  dataset[ , ctrx_total_atencion         := rowSums( cbind( ccallcenter_trx, chomebanking_trx, ccajas_trx, ctrx_quarter, cmobile_app_trx) , na.rm=TRUE ) ]
  dataset[ , ctrx_total_dinero           := rowSums( cbind( ctarjeta_debito_trx,  ctarjeta_visa_trx, ctarjeta_master_trx, cpayroll_trx, cpayroll2_trx, catm_trx, catm_trx_other) , na.rm=TRUE ) ]
  dataset[ , mtrx_total_dinero           := rowSums( cbind( mautoservicio,  mtarjeta_visa_consumo, mtarjeta_master_consumo, mpayroll, mpayroll2, matm, matm_other) , na.rm=TRUE ) ]
  dataset[ , mont_by_cant_trx            := mtrx_total_dinero  / ctrx_total_dinero ]
